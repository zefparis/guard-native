 ...
 *
 * Per-session instance created by the enrollment orchestrator.
 * Reset guaranteed at START. No global state.
 *
 * Ported from pulseguard-app/src/demoguard/behavior/behaviorSession.ts.
 * Adapted for React Native: window/navigator replaced with Platform check.
 * performance.now() is available globally in React Native.
 *
...
export class BehaviorSession {
  ...
  private detectTouchSupport(): void {
    // React Native: touch is always supported on native platforms (iOS/Android).
    // On web (expo-web), check window.ontouchstart.
